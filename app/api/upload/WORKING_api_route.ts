...
function jitterToPublicLocation(lat: number, lng: number, neighborhood: any) {
  // Jitter within neighborhood (±50-100 meters for privacy)
  // 0.001 degrees ≈ 111 meters
  const jitterAmount = 0.0008; // ~90 meters
  ...
  return {
    lat: lat + offsetLat,
    lng: lng + offsetLng,
    publicLocation: neighborhood.name
  };
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (jsonFile) {
      ...
      if (gps) source = 'google_takeout_json';
    }
    ...
    const jittered = jitterToPublicLocation(gps.lat, gps.lng, neighborhood);
